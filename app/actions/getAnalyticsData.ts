...
1. **성장성 진단**: 1년 전과 비교하여 회사가 성장하고 있는지, 정체 중인지 진단해주세요. (신규 유입 vs 재방문 고려)
2. **핵심 지표(KPI) 제안**: [Events] 데이터를 기반으로, 이 서비스가 가장 집중해서 관리해야 할 '단 하나의 지표(North Star Metric)'가 무엇인지 추론하고 그 이유를 설명하세요. (예: page_view보다 file_download가 중요한지 등)
3. **병목 구간 발견**: 유입은 되는데 전환(이벤트)이 일어나지 않는 구간이나, 이탈이 심한 기기/환경이 있는지 찾아주세요.
4. **결론**: CEO가 당장 실행해야 할 3가지 액션 아이템을 **우선순위 순서대로** 제안하세요.
`;
